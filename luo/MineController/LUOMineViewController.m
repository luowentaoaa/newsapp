...
#import "LUOMineViewController.h"
#import <SDWebImage.h>
#import "LUOLogin.h"

@interface LUOMineViewController ()<UITableViewDelegate, UITableViewDataSource>
...
@implementation LUOMineViewController
...
- (void)tableView:(UITableView *)tableView willDisplayHeaderView:(UIView *)view forSection:(NSInteger)section{
    if (![[LUOLogin sharedLogin] isLogin]) {
        [_headerImageView setImage:[UIImage imageNamed:@"icon.bundle/icon.png"]];
    }else{
       [self.headerImageView sd_setImageWithURL:[NSURL URLWithString:[LUOLogin sharedLogin].avatarUrl]];
    }
}

- (void)tableView:(UITableView *)tableView willDisplayCell:(UITableViewCell *)cell forRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.row == 0) {
        cell.textLabel.text =  [[LUOLogin sharedLogin] isLogin] ? [LUOLogin sharedLogin].nick: @"昵称";
    } else {
        cell.textLabel.text = [[LUOLogin sharedLogin] isLogin] ? [LUOLogin sharedLogin].address: @"地区";
    }
}
...
- (void)_tapImage {
    ...
    if (![[LUOLogin sharedLogin] isLogin]) {
        //没有登录的时候拉起登录
        [[LUOLogin sharedLogin] loginWithFinishBlock:^(BOOL isLogin) {
